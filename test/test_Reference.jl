...
@testset "reference_measure_basismap" begin
    ...
    l = length(basislis)
    output11 = FibonacciChain.reference_measure_basismap.(
        Ref(model),
        T,
        T,
        τ,
    ...
    output21 = FibonacciChain.reference_measure_basismap.(
        Ref(model),
        T,
        T,
        τ,
    ...
    output31 = FibonacciChain.reference_measure_basismap.(
        Ref(model),
        T,
        T,
        τ,
    ...
    @test all([all(output11[i] == output12[i]) for i = 1:l])
    @test all([all(output21[i] == output22[i]) for i = 1:l])
    @test all([all(output31[i] == output32[i]) for i = 1:l])

    extended_basis = FibonacciChain.build_extended_basis(1, basislis)
    output13 = FibonacciChain.reference_measure_basismap.(
        Ref(model),
        T,
        BitStr{N+1,Int},
        τ,
    ...
    output23 = FibonacciChain.reference_measure_basismap.(
        Ref(model),
        T,
        BitStr{N+1,Int},
        τ,
    ...
    output33 = FibonacciChain.reference_measure_basismap.(
        Ref(model),
        T,
        BitStr{N+1,Int},
        τ,
    ...
    @test all([all([all(output13[i+j*l] == output12[i]) for i = 1:l]) for j = 0:1])
    @test all([all([all(output23[i+j*l] == output22[i]) for i = 1:l]) for j = 0:1])
    @test all([all([all(output33[i+j*l] == output32[i]) for i = 1:l]) for j = 0:1])

    extended_basis2 = FibonacciChain.build_extended_basis(2, basislis)
    output14 = FibonacciChain.reference_measure_basismap.(
        Ref(model),
        T,
        BitStr{N+2,Int},
        τ,
    ...
    output24 = FibonacciChain.reference_measure_basismap.(
        Ref(model),
        T,
        BitStr{N+2,Int},
        τ,
    ...
    output34 = FibonacciChain.reference_measure_basismap.(
        Ref(model),
        T,
        BitStr{N+2,Int},
        τ,
    ...
    @test all([all([all(output14[i+j*l] == output12[i]) for i = 1:l]) for j = 0:3])
    @test all([all([all(output24[i+j*l] == output22[i]) for i = 1:l]) for j = 0:3])
...
@testset "reference_measure_basismap_Ising" begin
    ...
    extended_basis = FibonacciChain.build_extended_basis(1, basislis)
    output13zz = FibonacciChain.reference_measure_basismap.(
        Ref(model_ZZ),
        T,
        BitStr{N+1},
        τ,
    ...
    output23zz = FibonacciChain.reference_measure_basismap.(
        Ref(model_ZZ),
        T,
        BitStr{N+1},
        τ,
    ...
    output33zz = FibonacciChain.reference_measure_basismap.(
        Ref(model_ZZ),
        T,
        BitStr{N+1},
        τ,
    ...
    @test all([all([all(output13zz[i+j*l] == output12zz[i]) for i = 1:l]) for j = 0:1])
    @test all([all([all(output23zz[i+j*l] == output22zz[i]) for i = 1:l]) for j = 0:1])
    @test all([all([all(output33zz[i+j*l] == output32zz[i]) for i = 1:l]) for j = 0:1])
end
...
@testset "reference_measuremap" begin
    ...
    output13 = FibonacciChain.reference_measuremap(
        model,
        τ,
        add_st,
        1,
    ...
    output23 = FibonacciChain.reference_measuremap(
        model,
        τ,
        add_st,
        2,
    ...
    output13 = FibonacciChain.reference_measuremap(
        ...
        false,
        k_old = 0,
        extended_basis = anyon_basis(model),
    )
    output23 = FibonacciChain.reference_measuremap(
        model,
        τ,
...
@testset "concat_bell_pair" begin
    ...
    bell_st =
        FibonacciChain.concat_bell_pair(N, st, basis_F, ext_basis, site_idx = 1, k_old = 0)
...
@testset "reference_boundary_evolution" begin
    ...
    boundary_evo_Ising =
        reference_boundary_evolution(model_Ising, add_st, config, sample, layer_idx = 2) # Need to note here must be layer_idx=2 for ZZ measurement, otherwise always measure X
    output1, free_energy = boundary_evo_Ising.state, boundary_evo_Ising.free_energy
...
@testset "reference_apply_measurement_layer" begin
    ...
    measure_outcome = FibonacciChain._reference_apply_measurement_layer(
        model,
        τ,
        add_st,
        sample,
...
@testset "reference_bulk_evolution" begin
    ...
    @test free_energy ≈
          [2.617994480798357, 2.563763819200177, 1.415477184509482, 2.8031245965479576]
end
...
@testset "_reference_born_measure" begin
    ...
    ext_basis = FibonacciChain.build_extended_basis(1, anyon_basis(model))
    mo = FibonacciChain._reference_born_measure(
        model,
        add_st,
        measure_config,
        extended_basis = ext_basis,
...
@testset "_reference_sample_measure" begin
    ...
    ext_basis = FibonacciChain.build_extended_basis(1, anyon_basis(model))
    measure_outcome = FibonacciChain._reference_sample_measure(
        model,
        add_st,
        samples,
        measure_config,
...
@testset "spatial_corr_matrix and reference rdm" begin
    ...
    mes[1] = 1/√2
    mes[end] = 1/√2
    sclis = [spatial_correlation(model_IsingZ, mes, i, j) for i = 1:N for j = 1:N if j!=i]
    ...
    sclis_ref = [spatial_correlation(model_IsingZ, ρ, i, j) for i = 1:N for j = 1:N if j!=i]
...
function compute_post_selection_Ising(
    L::Int64,
    τ::Float64,
    t::Int64 = 5L,
    δt::Int = 2;
    ...
    ref_sample =
        sign ? BitMatrix(ones(Int, 2*(t+δt+t), L)) : BitMatrix(zeros(Int, 2*(t+δt+t), L))
    ...
    if δt == 0
        ...
        ref_config =
            MeasureConfig(τ = τ, t₂ = t+δt, t₁ = t, mode = :sample, x₂ = L÷2+1, x₁ = L÷2+1)
    end
