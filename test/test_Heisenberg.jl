...
@testset "HeisenbergAnyon basis" begin
    N = 4
    basis_obc = anyon_basis(AnyonModel(HeisenbergAnyon(), N, pbc = false))
    ...
    @test anyon_basis(AnyonModel(HeisenbergAnyon(), N, pbc = true)) == basis_obc
end
...
@testset "actingHam Heisenberg" begin
    ...
    model = AnyonModel(HeisenbergAnyon(), 2, pbc = false)
    ...
    model = AnyonModel(HeisenbergAnyon(), 2, pbc = false, h = 0.5)
    output = FibonacciChain.actingHam(model, bit"00")
    ...
    model = AnyonModel(HeisenbergAnyon(), 2, pbc = true)
    output = FibonacciChain.actingHam(model, bit"01")
...
@testset "anyon_ham Heisenberg" begin
    ...
    model = AnyonModel(HeisenbergAnyon(), 3, pbc = false, J = J, Jz = Jz, h = h)
    @test anyon_ham(model) ≈ H_obc_ref

    # PBC adds the (3,1) bond
    H_pbc_ref = H_obc_ref + J * (X ⊗ Id ⊗ X) + J * (Y ⊗ Id ⊗ Y) + Jz * (Z ⊗ Id ⊗ Z)
    model_pbc = AnyonModel(HeisenbergAnyon(), 3, pbc = true, J = J, Jz = Jz, h = h)
    @test anyon_ham(model_pbc) ≈ H_pbc_ref

    # Jz defaults to J (isotropic XXX)
    @test anyon_ham(AnyonModel(HeisenbergAnyon(), 3, pbc = false, J = J)) ≈
          anyon_ham(AnyonModel(HeisenbergAnyon(), 3, pbc = false, J = J, Jz = J))

    # Singlet/triplet spectrum of the isotropic two-site chain
    @test eigvals(anyon_ham(AnyonModel(HeisenbergAnyon(), 2, pbc = false))) ≈
          [-3.0, 1.0, 1.0, 1.0]

    # Hermiticity and dense/sparse consistency
    H = anyon_ham(AnyonModel(HeisenbergAnyon(), 4, pbc = true, Jz = 0.3, h = 0.2))
    @test ishermitian(H)
    H_sparse = anyon_ham_sparse(AnyonModel(HeisenbergAnyon(), 4, pbc = true, Jz = 0.3, h = 0.2))
...
@testset "anyon_ham Heisenberg momentum sectors" begin
    ...
    model = AnyonModel(HeisenbergAnyon(), 4, pbc = true, J = 1.0, h = 0.3)
...
@testset "Heisenberg measurement operators" begin
    ...
    for mop in (:X, :ZZ)
        model_h = AnyonModel(HeisenbergAnyon(), N, pbc = true, measure_operator = mop)
        model_i = AnyonModel(IsingAnyon(), N, pbc = true, measure_operator = mop)
    ...
    model = AnyonModel(HeisenbergAnyon(), N, pbc = true)
