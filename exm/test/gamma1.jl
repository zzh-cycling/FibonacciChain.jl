...
function samples_generate(L::Int64, τ::Float64, index::Int64, seed::Int64, D::Int64 = 120L)
    ...
    @time sample_measured_states, sample, sample_free_energy =
        Bulkmeasure(L, τ, st, D, rng, true)

    halfchain_EE_tlis =
        [ee(rdm_Fibo(L, collect(1:div(L, 2)), j)) for j in sample_measured_states]
    final_state = sample_measured_states[end]
    final_EElis = eelis_Fibo_state(L, final_state)


    save(
        "./exm/data/Bulk_measure/Observable_monitored_dynamics/L$(L)/τ$(τ)/D$(div(D,L))_Samples$(index).jld",
        "halfchain_EE_tlis",
        halfchain_EE_tlis,
        "final_EElis ",
        final_EElis,
...
function sample_continue_calculate(
    ...
    sample_measured_states, sample, sample_free_energy =
        Bulkmeasure(L, τ, st[end-1], additional_layers, rng)
    halfchain_EE_tlis =
        [ee(rdm_Fibo(L, collect(1:div(L, 2)), j)) for j in sample_measured_states]
    final_state = sample_measured_states[end]
    final_EElis = eelis_Fibo_state(L, final_state)
    ...
    save(
        "./exm/data/Bulk_measure/Observable_monitored_dynamics/L$(L)/τ$(τ)/D$(div(D+additional_layers,L))_Samples$(index).jld",
        "halfchain_EE_tlis",
        halfchain_EE_tlis,
        "final_EElis ",
        final_EElis,
    ...
    save(
        "exm/data/Bulk_measure/Samples_monitored_dynamics/L$(L)/τ$(τ)/D$(div(D+additional_layers,L))_Samples$(index).jld",
        "sample",
        sample,
        "sample_free_energy",
        sample_free_energy,
...
function samples_collect(L::Int64, τ::Float64, D::Int64 = 120L)
    ...
    for i = 1:samples_num
        ...
        @time sample, sample_free_energy, seed = load(
            "exm/data/Bulk_measure/Samples_monitored_dynamics/L$(L)/τ$(τ)/D$(div(D,L))_Samples$(i).jld",
            "sample",
            "sample_free_energy",
            "seed",
        )
    ...
    save(
        "exm/data/Bulk_measure/monitored_dynamics_ensemble_L$(L)_τ$(τ)_D$(div(D,L)).jld",
        "ensemble",
        ensemble,
...
function Observable_collect(L::Int64, τ::Float64, D::Int64 = 120L)
    ...
    for i = 1:samples_num
        ...
        halfchain_EE_tlis, final_EElis, seed, sample_free_energy = load(
            "./exm/data/Bulk_measure/Observable_monitored_dynamics/L$(L)/τ$(τ)/D$(div(D,L))_Samples$(i).jld",
            "halfchain_EE_tlis",
            "final_EElis ",
            "seed",
            "sample_free_energy",
    ...
    save(
        "exm/data/Bulk_measure/monitored_EE_FEdynamics_L$(L)_τ$(τ)_D$(div(D,L)).jld",
        "average_EE_tlis",
        average_EE_tlis,
        "stderr_EE_tlis",
        stderr_EE_tlis,
