...
function samples_generate(L::Int64, τ::Float64, index::Int64, seed::Int64, D::Int64=5L)
    ...
    save("./exm/data/Bulk_measure/Observable_monitored_dynamics/1010L$(L)/τ$(τ)_D$(div(D,L))_Samples$(index).jld", "halfchain_EE_tlis", halfchain_EE_tlis, "final_EElis ", final_EElis, "seed", seed, "sample_free_energy", sample_free_energy)

    save("exm/data/Bulk_measure/Samples_monitored_dynamics/1010L$(L)/τ$(τ)_D$(div(D,L))_Samples$(index).jld", "sample", sample, "sample_free_energy", sample_free_energy, "seed", seed)
...
function samples_collect(L::Int64, τ::Float64, D::Int64=5L)
    ...
    ensemble_seed = Vector{Int64}(undef, samples_num)
     for i in 1:samples_num
        @show i
        @time sample, sample_free_energy, seed = load("exm/data/Bulk_measure/Samples_monitored_dynamics/1010L$(L)/τ$(τ)_D$(div(D,L))_Samples$(i).jld", "sample", "sample_free_energy", "seed")
        ensemble[i] = sample
        ensemble_free_energy[i] = sample_free_energy
        ensemble_seed[i] = seed
    end

    save("exm/data/Bulk_measure/monitored_dynamics_ensemble_1010L$(L)_τ$(τ)_D$(div(D,L)).jld", "ensemble", ensemble, "ensemble_free_energy", ensemble_free_energy, "ensemble_seed", ensemble_seed)
end

function samples_calculate(L::Int64, τ::Float64, index::Int64, D::Int64=5L)
    st=zeros(length(Fibonacci_basis(L)))
    st[1] = 1.0
...
function Observable_collect(L::Int64, τ::Float64, D::Int64=5L)
    samples_num = 2000
    ensemble_free_energy = Vector{Vector{Float64}}(undef, samples_num)
    ensemble_seed = Vector{Int64}(undef, samples_num)
    ...
    for i in 1:samples_num
        @show i
        halfchain_EE_tlis, final_EElis, seed, sample_free_energy = load("./exm/data/Bulk_measure/Observable_monitored_dynamics/1010L$(L)/τ$(τ)_D$(div(D,L))_Samples$(i).jld", "halfchain_EE_tlis", "final_EElis ", "seed",  "sample_free_energy")
    ...
    save("exm/data/Bulk_measure/monitored_EE_FEdynamics_1010L$(L)_τ$(τ)_D$(div(D,L)).jld", "average_EE_tlis", average_EE_tlis, "stderr_EE_tlis", stderr_EE_tlis, "bulk_meanEElis", bulk_meanEElis, "ensemble_stderr_EElis",ensemble_stderr_EElis, "ensemble_free_energy", ensemble_free_energy, "ensemble_seed", ensemble_seed)
end

function ensemble_calculate(L::Int64, τ::Float64, D::Int64=5L)
    st=zeros(length(Fibonacci_basis(L)))
    st[1] = 1.0
