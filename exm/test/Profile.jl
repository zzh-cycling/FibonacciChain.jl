...
plot(
    [elapsed_time2, elapsed_time3, elapsed_time4] ./ elapsed_time1,
    [
        sqrt(mean((EElis2 .- EElis1) .^ 2)),
        sqrt(mean((EElis3 .- EElis1) .^ 2)),
        sqrt(mean((EElis4 .- EElis1) .^ 2)),
    ],
    yaxis = :log,
    label = L"Δ ee",
    marker = :circle,
    title = "Execution Time vs Error for different Cutoff and Maxdim",
    xlabel = L"t/t_{max}",
    ylabel = "Error",
    legend = :topright,
)
plot!(
    ...
    marker = :circle,
    [
        sqrt(mean((bulk_weights2 .- bulk_weights1) .^ 2)),
        sqrt(mean((bulk_weights3 .- bulk_weights1) .^ 2)),
        sqrt(mean((bulk_weights4 .- bulk_weights1) .^ 2)),
    ],
)
...
function run_mps_benchmark(
    ...
    try
        ...
        if exact_results === nothing
            exact_free_energy, exact_ee, _, _, _ =
                get_exact_results(N, τ, D; rng_seed = rng_seed, pbc = pbc)
        else
...
function parameter_sweep(N = 20, τ = 1.0, D = 10; rng_seed = 100, pbc = true)
    ...
    exact_free_energy, exact_ee, _, _, _ =
        get_exact_results(N, τ, D; rng_seed = rng_seed, pbc = pbc)
    @printf(
        "Exact results: Final Free Energy = %.6f, EE = %.4f ± %.4f\n",
        exact_free_energy[end],
        mean(exact_ee),
        std(exact_ee)
    )
    println()
    ...
    for cutoff in cutoffs
        for maxdim in maxdims
            current_combo += 1
            print(
                "[$current_combo/$total_combinations] Testing cutoff=$cutoff, maxdim=$maxdim... ",
            )

            result = run_mps_benchmark(
                N,
                τ,
                D,
                cutoff,
            ...
            if result.success
                @printf(
                    "✓ Time: %.3fs, ΔF: %.2e, ΔEE: %.2e\n",
                    result.execution_time,
                    result.free_energy_error,
                    result.ee_error
                )
            else
...
function analyze_results(results::Vector{BenchmarkResult}, exact_results = nothing)
    ...
    if exact_results !== nothing
        exact_free_energy, exact_ee = exact_results
        @printf(
            "Exact results: Free Energy = %.6f, EE = %.4f ± %.4f\n",
            exact_free_energy[end],
            mean(exact_ee),
            std(exact_ee)
        )
    ...
    for (i, result) in enumerate(precision_sorted[1:min(5, end)])
        @printf(
            "%d. cutoff=%.0e, maxdim=%d: ΔF=%.2e, time=%.3fs, memory=%.1fMB\n",
            i,
            result.cutoff,
            result.maxdim,
            result.free_energy_error,
    ...
    for (i, result) in enumerate(ee_precision_sorted[1:min(5, end)])
        @printf(
            "%d. cutoff=%.0e, maxdim=%d: ΔEE=%.2e, time=%.3fs, memory=%.1fMB\n",
            i,
            result.cutoff,
            result.maxdim,
            result.ee_error,
    ...
    for (i, result) in enumerate(speed_sorted[1:min(5, end)])
        @printf(
            "%d. cutoff=%.0e, maxdim=%d: time=%.3fs, ΔF=%.2e, ΔEE=%.2e\n",
            i,
            result.cutoff,
            result.maxdim,
            result.execution_time,
    ...
    for (i, result) in enumerate(memory_sorted[1:min(5, end)])
        @printf(
            "%d. cutoff=%.0e, maxdim=%d: memory=%.1fMB, time=%.3fs, ΔF=%.2e\n",
            i,
            result.cutoff,
            result.maxdim,
            result.memory_usage,
    ...
    # 找到满足精度要求的结果
    accurate_results = filter(
        r -> r.free_energy_error < fe_threshold && r.ee_error < ee_threshold,
        successful_results,
    )
    ...
    if !isempty(accurate_results)
        ...
        @printf(
            "Best for production (ΔF<%.0e, ΔEE<%.0e): cutoff=%.0e, maxdim=%d (time=%.3fs)\n",
            fe_threshold,
            ee_threshold,
            fastest_accurate.cutoff,
            fastest_accurate.maxdim,
            fastest_accurate.execution_time
        )
    else
        println(
            "No results meet the accuracy threshold. Consider looser requirements or higher maxdim.",
        )
    end

    # 最佳平衡（时间和精度的加权几何平均）
    best_balance = argmin(
        r -> sqrt(r.execution_time * (r.free_energy_error + r.ee_error)),
        successful_results,
    )
    @printf(
        "Best overall balance: cutoff=%.0e, maxdim=%d (time=%.3fs, ΔF=%.2e, ΔEE=%.2e)\n",
        best_balance.cutoff,
        best_balance.maxdim,
        best_balance.execution_time,
        best_balance.free_energy_error,
        best_balance.ee_error
    )

    # 快速原型设计
    fast_prototype = speed_sorted[1]
    @printf(
        "For fast prototyping: cutoff=%.0e, maxdim=%d (time=%.3fs, ΔF=%.2e)\n",
        fast_prototype.cutoff,
        fast_prototype.maxdim,
        fast_prototype.execution_time,
    ...
    @printf(
        "For high precision: cutoff=%.0e, maxdim=%d (ΔF=%.2e, ΔEE=%.2e)\n",
        high_precision.cutoff,
        high_precision.maxdim,
        high_precision.free_energy_error,
        high_precision.ee_error
    )
...
function multi_seed_benchmark(
    ...
    for cutoff in cutoffs
        for maxdim in maxdims
            ...
            for (i, seed) in enumerate(seeds)
                current_run += 1
                print(
                    "[$current_run/$total_runs] Testing cutoff=$cutoff, maxdim=$maxdim, seed=$seed... ",
                )
    ...
    for cutoff in cutoffs
        for maxdim in maxdims
            ...
            if !isempty(successful)
                ...
                push!(
                    summary_results,
                    (cutoff, maxdim, mean_time, mean_fe_error, mean_ee_error, success_rate),
                )
            else
    ...
    if !isempty(summary_results)
        ...
        if !isempty(reliable_results)
            ...
            @printf(
                "Best accuracy: cutoff=%.0e, maxdim=%d (ΔF=%.2e, ΔEE=%.2e)\n",
                best_accuracy[1],
                best_accuracy[2],
                best_accuracy[4],
                best_accuracy[5]
            ...
            @printf(
                "Best speed: cutoff=%.0e, maxdim=%d (time=%.3fs)\n",
                best_speed[1],
                best_speed[2],
                best_speed[3]
            )
            ...
            @printf(
                "Best balance: cutoff=%.0e, maxdim=%d (time=%.3fs, errors=%.2e)\n",
                best_balance[1],
                best_balance[2],
                best_balance[3],
                best_balance[4] + best_balance[5]
            )
        else
...
successful_results, pareto_results, best_balance =
    analyze_results(results, (exact_free_energy, exact_ee));


param_results, summary = multi_seed_benchmark(
    20,
    1.0,
    100,
    1:10,
    [1e-12, 1e-10, 1e-10, 1e-8, 1e-6],
    [50, 60, 100, 150, 200],
)
