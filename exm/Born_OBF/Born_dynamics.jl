...
@everywhere begin
    ...
    function born_dynamics_samples_generate(L::Int64, λ::Float64, ind::Int64, index::Int64)
        ...
        try
            ...
            save(
                "./exm/data/OBF/Born_dynamics_records/L$(L)/τ$(τ)/λ$(λ)/t$(t)_samples$(index).jld2",
                "halfchain_EE_tlis",
                halfchain_EE_tlis,
                "final_EElis",
                final_EElis,
...
if length(ARGS) == 0
    ...
    elseif mode == 2
        ...
        failed_tasks = [
            (L_res, λ_res, τ_res, idx_res, error) for
            (L_res, λ_res, τ_res, idx_res, status, error) in results if status != :success
        ]
