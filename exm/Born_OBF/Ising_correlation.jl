...
function conformal_tower(N)
    ...
    fig = plot(
        title = "Conformal Tower for Ising CFT",
        xlabel = "Momentum k",
        ylabel = "(E - E0) * (N / 2πv)",
        legend = false,
    )
    for n = 1:size(energy_spectrum_momentum, 2)
...
function ising_correlation(N)
    ...
    energy, states = Arpack.eigs(H_sparse, nev = 1, which = :SR)
    GS = states[:, 1]
    corr_σσ = [abs(anyon_correlation(model, GS, σ, i, j)) for i = 1:N, j = 1:N]
...
function correlation_scaling_dimension(N)
    ...
    fig = plot(
        ...
        seriestype = :scatter,
        xlabel = L"r",
        ylabel = L"C(r) = \langle \sigma_0 \sigma_r \rangle",
        title = "Spin-Spin Correlation Function Scaling",
        label = false,
    )
    ...
    plot!(
        ...
        r_fit,
        C_fit,
        label = "Fit: Δ=$(round(Δ_fit, digits=4))",
        linewidth = 2,
        color = :red,
    )
