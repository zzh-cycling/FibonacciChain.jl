...
function Boundarypost_selection_scaling(N)
    ...
    final_sequence_m_tau_lis = Vector{Vector{Symbol}}(undef, length(τlis))
    ...
    for (idx, τ) in enumerate(τlis)
        ...
        @time final_state_p, final_sequence_p, total_weight_p = Boundarypost_selection(N, τ, antiGS, measurement_sites, :p)
        final_state_m, final_sequence_m, total_weight_m = Boundarypost_selection(N, τ, antiGS, measurement_sites, :m)
