...
function Born_Sampling_free_energy_tau_lis(N, num_samples::Int=1000)
    γlis = vcat(collect(0.0:0.05:0.95), [0.99, 0.999])
    τlis = atanh.(γlis)
    ...
    for (idx, τ) in enumerate(τlis)
        ...
        # cent, fig = fitCCEntEntScal(average_FE_lis, mincut=2, pbc=true)
        # display(fig)
    end

    save("./exm/data/Born_Sampling_FE_tau_lis_N$(N).jld", "average_FE_tau_lis", average_FE_tau_lis, "variance_FE_tau_lis", variance_FE_tau_lis)
end
...
if length(ARGS) == 0
    ...
    for arg in ARGS
        ...
        Born_Sampling_free_energy_tau_lis(N)
    end
end
