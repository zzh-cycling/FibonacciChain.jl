...
function samples_generate(L::Int64, τ::Float64, index::Int64, seed::Int64, D::Int64=20L)
    ...
    @time sample_measured_states, sample, sample_free_energy = Bulkmeasure(L, τ, st, D, rng, true, measure_class=:IsingX) 

    halfchain_EE_tlis = [ee(rdm_Fibo(L, collect(1:div(L,2)), j, measure_class = :IsingX)) for j in sample_measured_states]
    final_state = sample_measured_states[end]
    final_EElis = eelis_Fibo_state(L, final_state, measure_class = :IsingX)
