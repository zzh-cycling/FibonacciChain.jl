...
@everywhere begin
    ...
    const Y1_DYNAMICS_DATA_ROOT =
        joinpath("exm", "data", "Bulk_measure", "monitored_dynamics_y1")

    """Return the normalized projection of the all-τ path into the y=1 sector."""
    function initial_y1_exact(model)
        ...
        projected = (y_on_state .- yτ .* state) ./ (ϕ - yτ)
        projected_norm = norm(projected)
        projected_norm > 1e-14 || error("The initial state has zero y=1 weight")

        expected_weight = (1 + ϕ * (-inv(ϕ))^model.N) / (1 + ϕ^2)
        isapprox(projected_norm^2, expected_weight; atol = 1e-10, rtol = 1e-10) ||
            error(
                "Unexpected y=1 weight $(projected_norm^2); expected $expected_weight",
            )
    ...
    """
    function initial_y1_mps(model; cutoff::Float64 = 1e-14, maxdim::Int = 32)
        model.pbc || error("The topological y=1 sector requires periodic boundaries")
        N = model.N
    ...
    function y1_default_periods(backend::Symbol, L::Integer, τ_idx::Integer)
        if backend == :exact
    ...
    function simulate_y1_exact(
        L::Integer,
        τ_idx::Integer,
        seed::Integer;
        periods::Integer = y1_default_periods(:exact, L, τ_idx),
    )
        iseven(L) || error("L must be even, got $L")
        τ = τlis_ext[τ_idx]
        model = fib_model(L)
        initial_state = initial_y1_exact(model)
    ...
    function simulate_y1_mps(
        L::Integer,
        τ_idx::Integer,
        χ::Integer,
        seed::Integer;
        periods::Integer = y1_default_periods(:mps, L, τ_idx),
    )
        iseven(L) || error("L must be even, got $L")
        χ >= 5 || error("χ must be at least 5 to represent the exact y=1 initial MPS")
        τ = τlis_ext[τ_idx]
        model = fib_model(L)
        initial_state, sites = initial_y1_mps(model; maxdim = χ)
    ...
    function save_y1_trajectory(
        ...
        out_dir = y1_trajectory_dir(backend, L, τ_idx; χ = χ)
        mkpath(out_dir)
    ...
    function run_y1_task(task)
        backend, L, τ_idx, χ, seed, periods = task
        try
            result = if backend == :exact
                simulate_y1_exact(L, τ_idx, seed; periods = periods)
            else
                simulate_y1_mps(L, τ_idx, χ, seed; periods = periods)
            end
            output_path = save_y1_trajectory(
                backend,
                L,
                τ_idx,
                seed,
    ...
    function collect_y1_trajectories(
        backend::Symbol,
        L::Integer,
        τ_idx::Integer;
        χ::Union{Nothing,Integer} = nothing,
        periods::Integer = y1_default_periods(backend, L, τ_idx),
    )
        data_dir = y1_trajectory_dir(backend, L, τ_idx; χ = χ)
        files = sort(filter(
        ...
        isempty(files) &&
            error("No y=1 trajectory files with periods=$periods found in $data_dir")
        ...
        output_path = joinpath(
            data_dir,
            "EE_FEdynamics_L$(L)_gamma$(τ_idx)_periods$(periods)_y1.jld2",
        )
        JLD2.jldsave(
            output_path;
            backend = String(backend),
...
function print_y1_usage()
    println("Usage:")
    println("  julia --project=. -p N exm/Bulk_measure/monitored_dynamics_y1.jl exact L τ_idx seed_start seed_end [periods]")
    println("  julia --project=. -p N exm/Bulk_measure/monitored_dynamics_y1.jl mps L τ_idx χ seed_start seed_end [periods]")
    println("  julia --project=. exm/Bulk_measure/monitored_dynamics_y1.jl collect_exact L τ_idx [periods]")
    println("  julia --project=. exm/Bulk_measure/monitored_dynamics_y1.jl collect_mps L τ_idx χ [periods]")
end
...
if isempty(ARGS)
    ...
    if action ∈ (:exact, :mps)
        ...
        default_periods = y1_default_periods(action, L, τ_idx)
    ...
    elseif action == :collect_exact
        ...
        periods = length(ARGS) == 4 ?
                  parse(Int, ARGS[4]) : y1_default_periods(:exact, L, τ_idx)
        output_path = collect_y1_trajectories(
            :exact,
            L,
            τ_idx;
            periods = periods,
    ...
    elseif action == :collect_mps
        ...
        periods = length(ARGS) == 5 ?
                  parse(Int, ARGS[5]) : y1_default_periods(:mps, L, τ_idx)
        output_path = collect_y1_trajectories(
            :mps,
            L,
            τ_idx;
