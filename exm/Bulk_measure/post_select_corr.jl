...
function organize(τ::Float64, sign::Int64 = 1)
    ...
    for (i, L) in enumerate(Llis)
        ...
        spatial_corr = load(
            "exm/data/Bulk_measure/spatial_temporal_corr/L$(L)/τ$(τ)/D$(D)_ps$(sign)_0.jld",
            "spatial_corr",
        )
        scLlis[i] = spatial_corr
        for (j, δt) in enumerate(δtlis)
            temporal_corr = load(
                "exm/data/Bulk_measure/spatial_temporal_corr/L$(L)/τ$(τ)/D$(D)_ps$(sign)_$(δt).jld",
                "temporal_corr",
            )
    ...
    save(
        "exm/data/Bulk_measure/spatial_temporal_corr/stc$(sign)_τ$(τ)_L$(Llis[1])$(Llis[end])_t0$(δtlis[end]).jld",
        "scLlis",
        scLlis,
        "tcLlis",
        tcLlis,
...
function compute_post_selection(
    L::Int64,
    τ::Float64,
    t::Int64 = 5L,
    δt::Int64 = 2;
    ...
    ref_sample =
        sign ? BitMatrix(ones(Int, 2*(t + δt + t), length(2:2:L))) :
        BitMatrix(zeros(Int, 2*(t + δt + t), length(2:2:L)))
    ...
    save(
        "exm/data/Bulk_measure/spatial_temporal_corr/L$(L)/τ$(τ)/D$(div(D,L))_ps$(sign)_$(δt).jld",
        "temporal_corr",
        temporal_corr,
        "spatial_corr",
        spatial_corr,
...
function spatial_temporal_corr_varyingt(
    ...
    if entangle_way == :copy
        ...
        for (idx, Δt) in enumerate(tlis)
            ...
            ref_sample =
                sign ? BitMatrix(ones(Int, 2*(t + δt + Δt), L)) :
                BitMatrix(zeros(Int, 2*(Δt + δt + t), L))
            if δt == 0
                ref_config = MeasureConfig(
                    τ = τ,
                    mode = :sample,
        ...
        save(
            "exm/data/Bulk_measure/spatial_temporal_corr_varying/L$(L)/τ$(τ)/D$(div(D,L))_ps$(sign)_$(δt).jld",
            "temporal_corr_lis",
            temporal_corr_lis,
            "spatial_corr_lis",
            spatial_corr_lis,
...
function plot_stc_tlis(L::Int64 = 10, D::Int64 = 3, τ::Float64 = log(1+√2); sign::Int = 0)
    ...
    temporal_corr_lis, spatial_corr_lis, eelis = load(
        "exm/data/Bulk_measure/spatial_temporal_corr_varying/L$(L)/τ$(τ)/D$(D)_ps$(sign)_0.jld",
        "temporal_corr_lis",
        "spatial_corr_lis",
        "eelis",
    )
    ...
    for (idx, δt) in enumerate(δtlis)
        temporal_corr_lis, spatial_corr_lis, eelis = load(
            "exm/data/Bulk_measure/spatial_temporal_corr_varying/L$(L)/τ$(τ)/D$(D)_ps$(sign)_$(δt).jld",
            "temporal_corr_lis",
            "spatial_corr_lis",
            "eelis",
        )
        ...
        plot!(
            ...
            tlis,
            temporal_corr_lis,
            label = latexstring("(δx,δt) = (0, $(δt/L)L)"),
            color = c[idx+1],
            linewidth = 2,
        )
    end
...
function plot_tc(L::Int, D::Int = 10, τ::Float64 = log(1+√2))
    ...
    spatial_corr_lis1 = load(
        "exm/data/Bulk_measure/spatial_temporal_corr_varying/L$(L)/τ$(τ)/D$(D)_ps1_0.jld",
        "spatial_corr_lis",
    )
    sc1 = spatial_corr_lis1[end]
    for (idx, δt) in enumerate(δtlis)
        temporal_corr_lis, spatial_corr_lis = load(
            "exm/data/Bulk_measure/spatial_temporal_corr_varying/L$(L)/τ$(τ)/D$(D)_ps1_$(δt).jld",
            "temporal_corr_lis",
            "spatial_corr_lis",
        )
