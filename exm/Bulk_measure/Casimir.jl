...
function fit_casimir_central_charge(L_list = collect(8:2:24))
    ...
    fit =
        curve_fit(bulk_dynamics_model, L_list[1:end], (FE_lis ./ L_list)[1:end], [0.5, 0.5])
    fit_paras = fit.param
    ...
    fig = plot(
        ...
        linewidth = 2,
        label = latexstring(
            "c_{\\mathrm{Casimir}} = $(round(fit_paras[2], digits=decimal_places))",
        ),
        framestyle = :box,
    )
...
function fit_casimir_coef()
    ...
    for (idx, γ) in enumerate(γlis)
        ...
        for (idy, L) in enumerate(L_list)
            ...
            if isfile(data_path)
                bulk_meanEElis, bulk_FE, bulk_FE_stderr =
                    load(data_path, "bulk_meanEElis", "bulk_FE", "bulk_FE_stderr")
                FE_lis[idy] = bulk_FE
...
function plot_cc_Casimir()
    ...
    fig = plot(
        ...
        yerror = ccstderr_lis,
        xlabel = L"γ",
        ylabel = "Conformal data",
        label = latexstring("c_{\\mathrm{ent}}"),
    ...
    plot!(
        ...
        marker = :circle,
        color = palette(:tab10)[2],
        label = latexstring("c_{\\mathrm{Casimir}}"),
    )
