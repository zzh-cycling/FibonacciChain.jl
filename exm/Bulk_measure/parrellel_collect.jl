...
@everywhere function get_correlation_dynamics_D(τ, L)
    cfg = Dict(atanh(0.3) => 50L, atanh(0.4) => 40L, atanh(0.5) => 30L, atanh(0.6) => 20L)
    D = get(cfg, τ, 0)
    return D
end
...
@everywhere function dynamics_collect(arg::Tuple)
    ...
    for i = 1:samples_num
        ...
        try
            temporal_corr_lis, spatial_corr_lis, eelis = load(
                "exm/data/Bulk_measure/spatial_temporal_corr_varying_Born/L$(L)/τ$(τ)/dt$(δt)/D$(div(D1,L))_Samples$(i).jld",
                "temporal_corr_lis",
                "spatial_corr_lis",
                "eelis",
            )
    ...
    if success == samples_num
        save(
            "exm/data/Bulk_measure/spatial_temporal_corr_varying_Born/L$(L)/τ$(τ)/dt$(δt)_collect.jld",
            "average_temporal_corr",
            average_temporal_corr_tlis,
            "temporal_corr_stderr",
            temporal_corr_tlis_stderr,
            "average_spatial_corr",
            average_spatial_corr_tlis,
            "spatial_corr_stderr",
            spatial_corr_tlis_stderr,
            "average_EE",
            average_EE_tlis,
            "stderr_EE",
            stderr_EE_tlis,
...
@everywhere function data_compress(arg::Tuple)
    ...
    for i = 1:samples_num
        try
            temporal_corr, spatial_corr, S, sample_free_energy, sample = load(
                "exm/data/Bulk_measure/spatial_temporal_corr_Born/L$(L)/τ$(τ)/dt$(δt)/D$(div(D1,L))_Samples$(i).jld",
                "temporal_corr",
                "spatial_corr",
                "S",
    ...
    if success == samples_num
        save(
            "exm/data/Bulk_measure/spatial_temporal_corr_Born/L$(L)/τ$(τ)/compressed_dt$(δt)_data.jld",
            "temporal_corr_ensemble",
            temporal_corr_ensemble,
            "spatial_corr_ensemble",
            spatial_corr_ensemble,
            "S_ensemble",
            S_ensemble,
            "sample_free_energy_ensemble",
            sample_free_energy_ensemble,
            "sample_ensemble",
            Bit_sample_ensemble,
            "samples_num",
...
@everywhere function compute_parallel_dt_GC(L::Int64, τ::Float64, δt_list::Vector{Int})
    ...
    for δt_start = 1:batch_size:total_tasks
        jobs = []
...
@everywhere function pack_one_file(job::Tuple{String,Int})
    ...
    try
        data = load(path_jld)
        haskey(data, "sample_ensemble") ||
            return (path_jld, false, "no key sample_ensemble")
...
@everywhere function pack_all_δt(L, τ, D; delete_old::Bool = false)
    ...
    files = [
        joinpath(dir, "compressed_dt$(δt)_data.jld") for
        δt in vcat(2 .* collect((64*5):(64*5+63)))
    ]
    if isempty(files)
