...
@everywhere function compute_ratio(
    L::Int64,
    τ::Float64,
    index::Int64,
    D::Int64 = 16L,
    ...
    if δt == 0
        ref_config =
            MeasureConfig(τ = τ, t₂ = t, t₁ = t, rng = rng, mode = :Born, x₂ = L÷2+1)
        spatial = true
        temporal = false
    else
...
@everywhere function spatial_corrlis(L::Int64, τ::Float64, index::Int64, D::Int64 = 16L)
    ...
    for site in dlis
        ...
        ref_config =
            MeasureConfig(τ = τ, t₂ = t, t₁ = t, rng = rng, mode = :Born, x₂ = 1+site)
        ref_mo = reference_evolution(model, pre_mo.state, ref_config, ref_sample)
        sample_layer, sample_free_energy = ref_mo.samples, ref_mo.free_energys  # to compute temporal correlation, add ref qubit at site L/2+1
        spatial = true
        temporal = false
        ref_st = ref_mo.state
        spatial_corr, _ =
            ref_correlation(model, ref_st, spatial = spatial, temporal = temporal)
    ...
    save(
        "exm/data/Bulk_measure/spatial_corr_Born/L$(L)/τ$(τ)/D$(div(D,L))_Samples$(index).jld",
        "spatial_corr_lis",
        spatial_corr_lis,
        "Slis",
        Slis,
...
function spatial_corrlis_collect(L, τ, mode::Int = 1)
    ...
    if mode==1
        ...
        old_spatial_corr_ensemble, old_S_ensemble = load(
            "exm/data/Bulk_measure/spatial_corr_Born/L$(L)/τ$(τ)/compressed_dt0_data.jld",
            "spatial_corr_ensemble",
            "S_ensemble",
        )
        ...
        for (index) in indexlis
            ...
            data = load(
                "exm/data/Bulk_measure/spatial_temporal_corr_Born/L$(L)/τ$(τ)/dt0/D$(div(D,2L))_Samples$(index).jld",
            )
    ...
    save(
        "exm/data/Bulk_measure/spatial_corr_Born/L$(L)/τ$(τ)_D$(div(D,L))_spatial_corr.jld",
        "spatial_corr_avg",
        spatial_corr_avg,
        "Slis_avg",
        Slis_avg,
...
function compute_parallel_multiple_dt(
    L::Int64,
    τ::Float64,
    seed_range::UnitRange{Int},
    D::Int64,
...
function compute_missing_tasks_parallel(filename::String)
    ...
    for batch_start = 1:batch_size:length(task_params)
        batch_end = min(batch_start + batch_size - 1, length(task_params))
        batch_tasks = task_params[batch_start:batch_end]

        println(
            "Processing batch $(div(batch_start-1, batch_size) + 1): tasks $batch_start to $batch_end",
        )
    ...
    println(
        "Success rate: $(round(total_success/(total_success + total_failed)*100, digits=1))%",
    )
end
