...
function get_δtL_Born(τ, L)
    ...
    elseif L == 18
        table = Dict(
            ...
            atanh(0.6) => vcat([2], collect(5:20), [25]),
    ...
    elseif L == 22
        table = Dict(
            ...
            atanh(0.6) => sort(vcat(collect(5:2:19), [2, 12, 16, 18, 20, 22])),
    ...
    elseif L == 24
        table = Dict(
            ...
            atanh(0.6) => sort(vcat(collect(10:2:20), [2, 5, 8, 17, 19, 22, 25])),
            atanh(1/√2) => (collect(1:15)),
        )
        δtlis = get(table, τ, collect(1:10))
    elseif L == 26
...
function get_cfg_params_Born(ind, L)
    ...
    t, step, start = get(cfg, ind, (12L, 6, 2L))
    inds = collect(1:step:t)
    avg_range = Int(2*start):(2t - 4)
    return t, inds, avg_range
end
...
function get_mps_params_Born(τind, L)
    ...
    t, step, start = get(cfg, τind, (8, 2, 1))
    inds = collect(1:step:(t*L))
    avg_range = Int(2start*L):(Int(2t*L)-4)
    return t, inds, avg_range
end
