...
function plot_inverse(τ::Float64)
    ...
    for (i, N) in enumerate(L_list)
        sample_measured_states, samples, sample_weights = load(
            "./exm/data/Born_Samples_N$(N)/Born_Samples_N$(N)_τ$(τ).jld",
...
function plot_compare(τ::Float64)
    ...
    for (i, N) in enumerate(L_list)
        ...
        @show (τ, i, N)
        sample_measured_states, samples, sample_weights =
            Sampling(N, τ, antiGS, measurement_sites, 10000)
        save(
            "exm/data/Samples_Compared/Born_Samples_N$(N)_τ$(τ).jld",
            "sample_measured_states",
            sample_measured_states,
            "samples",
            samples,
        ...
        totalFE_enumed =
            load("exm/data/Boundary_measure/Born_enumed_FE_N1020.jld", "totalFE_enumed")
...
function plot_compare_distribution()
    ...
    trajectorieslis, probabilitieslis = load(
        "./exm/data/Boundary_measure/Born_enumed_eelis_N10.jld",
        "trajectorieslis",
        "probabilitieslis",
    )
    for (idx, τ) in enumerate(τlis[end])
        sample_measured_states, samples, sample_weights = load(
            "exm/data/Samples_Compared/Born_Samples_N10_τ$(τ).jld",
            "sample_measured_states",
            "samples",
            "sample_weights",
        )
        ...
        decimal_value_enum = [
            ...
            j in binary_digits_enum
        ]
        binary_digits_samples = map.(symbol -> symbol == :p ? 0 : 1, samples)
        decimal_value_samples = [
            sum(d * 2^(length(j) - i) for (i, d) in enumerate(j)) for
            j in binary_digits_samples
        ]
        count_lis = [count(x->x==i, decimal_value_samples) for i = 0:31]
...
function plot_sample(N::Int64, τ::Float64)
    ...
    decimal_value =
        [sum(d * 2^(length(j) - i) for (i, d) in enumerate(j)) for j in binary_digits]
