...
function compute_correlations(L::Int; inds::Int64)
    ...
    for (i, d) in enumerate(dlis)
        ...
        q_corr_samples = [
            spatial_correlation(model, sample_measured_states[j], site1, site2) for
            j = 1:n_samples
        ]
    ...
    for (i, d) in enumerate(dlis[1:(end-1)])
        ...
        site2 = site1 + d
        m_corr_samples =
            spins[:, site1] .* spins[:, site2] .- spin_avg[site1] * spin_avg[site2]
        measure_corr[i] = mean(m_corr_samples)
        measure_corr_std[i] = std(m_corr_samples) / sqrt(n_samples)
    end

    save(
        "exm/data/Boundary_measure/spatial_corr_Born/correlation_L$(L)_τ$(τ).jld",
        "dlis",
        dlis,
        "quantum_corr",
        quantum_corr,
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    for (d, q, qs, m, ms) in zip(dlis, q_corr, q_std, m_corr, m_std)
        println(
            "   $d     |   $(round(q, digits=4)) ± $(round(qs, digits=4))   |   $(round(m, digits=4)) ± $(round(ms, digits=4))",
        )
    end
end
